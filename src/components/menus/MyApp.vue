...
<div style="height:inherit;">
  ...
  <div id="apps" class="contents">
    <div class="list st2" ref="mainScroll">
      <ul>
        <template v-for="item in serverApplication">
          <app-item
            :key="item.id"
            :id="item.id || ''"
            :status="item.status || ''"
            :title="item.getName() || item.names['default'] || ''"
            :imgSrc="item.iconSrc || ''"
            :badge="true"
          ></app-item>
        </template>
        <template v-for="item in newLocalApplication">
          <app-item
            :key="item.id"
            :id="item.id || ''"
            :status="item.status || ''"
        ...
        <template v-for="item in orginLocalApplication">
          <app-item
            :key="item.id"
            :id="item.id || ''"
            :status="item.status || ''"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters({
      ...
      newLocalApplication: 'newLocalApplication',
      orginLocalApplication: 'orginLocalApplication'
    })
  },
  methods: {
    onStart (e) {
